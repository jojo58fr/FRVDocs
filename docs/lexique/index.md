---
sidebar_position: 7
title: Lexique VTubing
sidebar_label: Lexique alphabétique
description: Une table des matières alphabétique pour tous les termes VTubing documentés.
slug: /lexique
---

## Lexique alphabétique du VTubing

<div class="lexicon-wiki">
  <p class="lexicon-summary">Ce dossier rassemble chaque terme important dans une fiche à part. Chaque section alphabétique regroupe les définitions et renvoie à leur fiche dédiée.</p>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">#</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/2-5d-idol">
        <div>
          <strong>Idol 2.5D</strong>
          <p>Idole VTuber qui se produit en virtuel et en live-action tout en gardant parfois son visage caché.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">A</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/asmrtist">
        <div>
          <strong>ASMRtist</strong>
          <p>Un VTuber spécialisé dans l'ASMR : des triggers doux (sleep, nettoyage d'oreilles) ou plus suggestifs, parfois aussi appelé ASMRist.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/avtuber">
        <div>
          <strong>AVTuber</strong>
          <p>Un VTuber qui produit du contenu R-18 en dehors de YouTube/Twitch, parfois associé à une agence de niche.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/affiliate">
        <div>
          <strong>Affiliate</strong>
          <p>Sur Twitch, un affilié est un streamer validé pour la monétisation ; chez Hololive cela désigne un talent inactif mais toujours rattaché pour des apparitions ou la marchandise.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/agence-vtuber">
        <div>
          <strong>Agence VTuber</strong>
          <p>Une agence VTuber est une entreprise qui recrute, forme et gère des créateurs à avatars virtuels, assurant technique, promotion, monétisation et organisation d'événements.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/akasupa">
        <div>
          <strong>Akasupa</strong>
          <p>Un don hautement valorisé sur YouTube, reconnaissable à son fond rouge (akai), autrement dit un superchat premium.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/anti">
        <div>
          <strong>Anti</strong>
          <p>Un troll ou un hater qui critique ouvertement un VTuber.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/avatar-virtuel">
        <div>
          <strong>Avatar virtuel</strong>
          <p>Un avatar est la représentation visuelle et identitaire d'une personne sur internet, incarnant pour un VTuber la personnalité, les émotions et l'univers que le créateur souhaite projeter.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">B</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/babiniku">
        <div>
          <strong>Babiniku</strong>
          <p>Un VTuber masculin qui incarne un personnage féminin en assumant son genre réel.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/black-company">
        <div>
          <strong>Black company</strong>
          <p>Une agence exploitante qui maltraite ses talents (non-paiement, doxxing, harcèlement).</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/bj">
        <div>
          <strong>Broadcast Jockey</strong>
          <p>Terme coréen pour désigner un livestreamer.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">C</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/capture-mouvement">
        <div>
          <strong>Capture de mouvement</strong>
          <p>La capture de mouvement enregistre les positions du corps via des capteurs pour animer fidèlement un avatar en appliquant ces données à son squelette numérique.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/carrier-pigeon">
        <div>
          <strong>Carrier pigeon</strong>
          <p>Un spectateur relayant les messages d'un stream vers un autre, souvent perçu comme du stream sniping.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/chroma-key">
        <div>
          <strong>Chroma key</strong>
          <p>Le chroma key consiste à filmer un sujet sur fond uni afin de l'isoler et le superposer à un décor numérique, utile en VTubing/Streaming pour créer des overlays stylisés.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/clipper">
        <div>
          <strong>Clipper</strong>
          <p>Une chaîne qui crée des clips à partir d'un stream, parfois officielle, parfois fan.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/clipping">
        <div>
          <strong>Clipping</strong>
          <p>Créer des vidéos à partir d'extraits d'un stream pour immortaliser les moments marquants.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/collab">
        <div>
          <strong>Collab</strong>
          <p>Une collaboration réunissant plusieurs VTubers dans un stream.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">D</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/delivery">
        <div>
          <strong>Delivery (haishin)</strong>
          <p>Traduction de 配信 : le terme japonais pour "stream" ou "diffusion."</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/diffusion-en-direct">
        <div>
          <strong>Diffusion en direct</strong>
          <p>Une diffusion en direct est un programme retransmis en temps réel, permettant aux spectateurs d'interagir instantanément via le chat et autres réactions.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/doxxing">
        <div>
          <strong>Doxxing</strong>
          <p>Révéler l'identité réelle d'un talent : acte condamné mettant fin à l'anonymat.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/dullahan">
        <div>
          <strong>Dullahan</strong>
          <p>Terme coréen pour un streamer sans visage visible, souvent avec une image statique.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/debut-stream">
        <div>
          <strong>Début (stream)</strong>
          <p>Le début (debut stream) est le premier live officiel d'un VTuber, où il présente son avatar, son personnage et lance officiellement sa carrière.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">E</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/endurance">
        <div>
          <strong>Endurance stream</strong>
          <p>Défi de stream souvent orienté sur un objectif de souscriptions ou de durée.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/enjou">
        <div>
          <strong>Enjou</strong>
          <p>Controverse massive en ligne, équivalent japonais de "getting flamed."</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">F</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/fan-name">
        <div>
          <strong>Fan name</strong>
          <p>Surnom collectif attribué aux fans d'un VTuber.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/fanart">
        <div>
          <strong>Fanart</strong>
          <p>Le fanart désigne les œuvres créées par les fans (dessins, animations, clips) pour représenter et promouvoir un VTuber.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/fandom">
        <div>
          <strong>Fandom</strong>
          <p>Le fandom est la communauté de fans d'un VTuber, rassemblée autour de plateformes et créant memes, clips et ressources partagées.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/fleshtuber">
        <div>
          <strong>Fleshtuber</strong>
          <p>Un streamer qui n'utilise pas de modèle virtuel.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/free-chat">
        <div>
          <strong>Free Chat</strong>
          <p>Salle de chat persistante créée via une diffusion programmée sur YouTube.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">G</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/gachikoi">
        <div>
          <strong>Gachikoi</strong>
          <p>Fan obsessionnel qui "tombe amoureux" d'un VTuber et dépasse souvent les limites.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/graduation">
        <div>
          <strong>Graduation</strong>
          <p>Stream d'adieu ou départ d'une agence qui marque la retraite du talent.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/guerrilla">
        <div>
          <strong>Guerrilla stream</strong>
          <p>Unstream non annoncé, souvent spontané.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/gyakutotsu">
        <div>
          <strong>Gyakutotsu</strong>
          <p>Généralement un stream anniversaire où le VTuber appelle des amis (reverse call-in).</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/generation">
        <div>
          <strong>Génération (gen)</strong>
          <p>Une génération désigne un groupe de VTubers lancés simultanément par une agence, partageant un branding ou identité commune pour faciliter leur promotion.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">I</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/idol">
        <div>
          <strong>Idol</strong>
          <p>Un idol est un artiste mis en avant pour son image et ses talents, une logique reprise par le VTubing à travers des VTubers présentés comme « idols virtuels » polyvalents.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/2-5d-idol">
        <div>
          <strong>Idol 2.5D</strong>
          <p>Idol VTuber qui se produit à la fois dans des espaces virtuels et en live-action, parfois sans révéler son visage.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">K</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/kami-oshi">
        <div>
          <strong>Kami-oshi</strong>
          <p>Super favori du fan, le coup de cœur ultime.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/kirinuki">
        <div>
          <strong>Kirinuki</strong>
          <p>Terme japonais pour désigner un clip vidéo.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/kojin-zei">
        <div>
          <strong>Kojin-zei</strong>
          <p>VTuber indépendant, souvent freelance ou dans un groupe indie.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/kusa">
        <div>
          <strong>Kusa</strong>
          <p>Exprime le rire, l'équivalent de "LOL" en japonais.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">L</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/live2d">
        <div>
          <strong>Live2D</strong>
          <p>Live2D est une technologie qui anime des illustrations 2D par morphing, permettant des mouvements naturels tout en préservant le style du dessin.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/liver">
        <div>
          <strong>Liver</strong>
          <p>Contraction japonaise de "livestreamer."</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/lore">
        <div>
          <strong>Lore</strong>
          <p>Les éléments fictionnels autour d'un VTuber.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">M</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/mama">
        <div>
          <strong>Mama</strong>
          <p>Le designer/illustrateur du VTuber.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/member">
        <div>
          <strong>Member</strong>
          <p>Spectateur payant à un abonnement de chaîne.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/mengen">
        <div>
          <strong>Mengen</strong>
          <p>Contenu réservé aux membres (member-exclusive).</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/moderator">
        <div>
          <strong>Moderateur</strong>
          <p>Personne chargée de modérer le chat, parfois un autre VTuber.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/3d-model">
        <div>
          <strong>Modèle 3D</strong>
          <p>Un modèle 3D est une version polygonale d'un avatar VTuber, créée dans un logiciel de modélisation pour permettre des mouvements complets et un rendu en profondeur.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/mute">
        <div>
          <strong>Mute</strong>
          <p>VTuber qui ne parle pas pendant ses streams.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">O</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/off-collab">
        <div>
          <strong>Off-collab</strong>
          <p>Collaboration en personne entre VTubers.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/oshi">
        <div>
          <strong>Oshi</strong>
          <p>Ton VTuber préféré.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/oshi-mark">
        <div>
          <strong>Oshi mark</strong>
          <p>Emoji officiel représentant le VTuber.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/otsu">
        <div>
          <strong>Otsu</strong>
          <p>"Otsukaresama" utilisé pour dire merci à la fin d'un stream.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">P</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/pngtuber">
        <div>
          <strong>PNGTuber</strong>
          <p>VTuber utilisant une image statique (PNG).</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/papa">
        <div>
          <strong>Papa</strong>
          <p>Designer/rigueur masculin du modèle VTuber.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/past-life">
        <div>
          <strong>Past life</strong>
          <p>Identité du talent avant le VTubing, habituellement taboue.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/pre-debut">
        <div>
          <strong>Pre-debut</strong>
          <p>Design révélé avant un début officiel.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/puppeteering">
        <div>
          <strong>Puppeteering</strong>
          <p>Le puppeteering consiste à manipuler un avatar pour obtenir un mouvement/expression précis.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">R</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/rtx-model">
        <div>
          <strong>RTX model</strong>
          <p>Cosplay d'un modèle VTuber en live-action, pour un rendu réaliste.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/raid">
        <div>
          <strong>Raid</strong>
          <p>Transfert automatique de spectateurs vers un autre stream.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/re-debut">
        <div>
          <strong>Re-debut</strong>
          <p>Nouvel arrival après un changement de modèle ou d'agence.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/redesign">
        <div>
          <strong>Redesign</strong>
          <p>Modification permanente du design du VTuber.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/relay">
        <div>
          <strong>Relay stream</strong>
          <p>Série de streams sur un thème, chacun passant la main au suivant.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/rigging">
        <div>
          <strong>Rigging (animation squelettique)</strong>
          <p>Le rigging consiste à créer un squelette virtuel hiérarchisé sous un modèle pour en contrôler les mouvements et les déformations lors de l'animation.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/reincarnation">
        <div>
          <strong>Réincarnation</strong>
          <p>VTuber retiré revenant sous une nouvelle identité.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">S</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/seiso">
        <div>
          <strong>Seiso</strong>
          <p>VTubers à l'image pure, sans vulgarité ni sujets salaces.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/stream-sniping">
        <div>
          <strong>Stream sniping</strong>
          <p>Regarder un stream pour obtenir un avantage dans un jeu.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/subathon">
        <div>
          <strong>Subathon</strong>
          <p>Stream marathon où chaque abonnement prolonge l'événement.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/super-chat">
        <div>
          <strong>Super Chat</strong>
          <p>Super Chat est une fonction YouTube permettant d'envoyer des messages payants mis en avant dans le chat d'un live, souvent lus en priorité par le créateur.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">T</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/talent">
        <div>
          <strong>Talent</strong>
          <p>La personne derrière l'avatar, considérée irremplaçable.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/teetee">
        <div>
          <strong>Teetee</strong>
          <p>Terme affectueux décrivant une relation chérie entre deux VTubers.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/totsumachi">
        <div>
          <strong>Totsumachi</strong>
          <p>Stream «凸待ち» où le VTuber attend des appels entrants.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">U</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/unarchived">
        <div>
          <strong>Unarchived</strong>
          <p>Stream sans VOD sauvegardée, souvent pour des raisons de droits.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/univers-alternatif">
        <div>
          <strong>Univers alternatif (AU)</strong>
          <p>Un univers alternatif (AU) est une version fictive ou modifiée d'un VTuber, utilisée pour explorer des variantes narratives ou conceptuelles de son personnage.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/utaite">
        <div>
          <strong>Utaite</strong>
          <p>Chanteur amateur qui publie des covers, souvent sur Niconico ou YouTube.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/utattemita">
        <div>
          <strong>Utattemita</strong>
          <p>Cover song (歌ってみた) ponctuant la scène utaite.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/utawaku">
        <div>
          <strong>Utawaku</strong>
          <p>Stream de chant ou de karaoké (歌枠).</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">V</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/vliver">
        <div>
          <strong>VLiver</strong>
          <p>Live-streamer virtuel sur IRIAM (Japon).</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/vod">
        <div>
          <strong>VOD (vidéo à la demande)</strong>
          <p>Une VOD est l'enregistrement d'un live ou d'une vidéo que les spectateurs peuvent revoir à tout moment après la diffusion.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/vtuber">
        <div>
          <strong>VTuber</strong>
          <p>Un VTuber est un créateur en ligne qui apparaît via un avatar numérique animé, mêlant streaming, narration et technologies d'animation en temps réel.</p>
        </div>
      </a>
      <a class="lexicon-card" href="/docs/lexique/vup">
        <div>
          <strong>VUP</strong>
          <p>Terme chinois pour VTuber, issu de "Virtual UP" sur bilibili.</p>
        </div>
      </a>
    </div>
  </section>
  <section class="lexicon-section">
    <div class="lexicon-letter-row">
      <span class="lexicon-letter-heading">Z</span>
      <span class="lexicon-letter-divider"></span>
    </div>
    <div class="lexicon-grid">
      <a class="lexicon-card" href="/docs/lexique/zatsudan">
        <div>
          <strong>Zatsudan</strong>
          <p>Stream de discussion libre (雑談), équivalent du "Just Chatting".</p>
        </div>
      </a>
    </div>
  </section>
  <p class="lexicon-summary">Chaque fiche approfondit le terme, les outils associés et les sources. Pour contribuer, suivez le lien vers la page concernée, ajustez le contenu, puis signalez les sources vérifiées.</p>
</div>
